...
def exercise04(more_temperatures, iot_sensor_points, a, b, c, d, e):
    ...
    temperatures[::-1].sort()   # Sorting in descending  order 
    samples = []
    samples = [temperatures[i] for i in range(len(temperatures)) if (i+1)%5==0 and i!=0] # Only keeping 5th element
    copy_of_samples = [samples[i] for i in range(len(samples))] # Shallow copy
    samples.sort() #ascending order
...
def exercise06(n):
     # This function will receive an arbitrary list of numbers of arbitrary size and 
     # find the average of those numbers. The size of the list may vary.
...
def exercise07(n):
    ...
    # ------ Place code below here \/ \/ \/ ------
    # Here I am doing two list comprehensions, 
...
def exercise08(s):
    # This function receives a string. The string should be casted to an int and 
    # then a float and 
...
def exercise09():
    ...
    # You will simply APPEND the dogs list with URLs of pics so that the list contains 11 URLs. 
...
def exercise10(sentence):
    ...
    # Exercise10 receives an arbitrary string. 
    # Return the sentence backwards with the cases inverted and spaces an underscore _, i.e. HelLo returns OlLEh
    reversed = ''
