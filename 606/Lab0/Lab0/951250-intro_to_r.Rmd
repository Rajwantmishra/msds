...
1.  What command would you use to extract just the counts of girls baptized? Try
    ...
    > We can user <b> arbuthnot$girls </b> : `r arbuthnot$girls `
...
2.  Is there an apparent trend in the number of girls baptized over the years?  
    ...
    >  Based on the below plot we can say (Blue color is data for Girls)
    > + that number of Girls baptized over the year is less than that of Boys but. it's very much following the same trend as that of boys. 
    > + Trends is directly proportional  to number of Boys Baptized but never exceeding that of boys.
 
```{r}
library(ggplot2)
ggplot(arbuthnot) +
 geom_line(mapping = aes(x=year,y=girls),color="blue") +
 geom_line(mapping = aes(x=year,y=boys))+
  xlab("Year") + ylab("Girls and Boys")
```
...
#### Answer 3
  ...
  > Proportion of boys over time can be seen data off the Boys over the year.
  

```{r}
plot(arbuthnot$year,arbuthnot$boys / (arbuthnot$boys + arbuthnot$girls),"l", col = "red")
```
...
## On Your Own {.tabset .tabset-fade .tabset-pills}
...
```{r load-present-data, include=FALSE}
...
str(present)
 ...
 plot(x = present$year, y = present$boys, type = "l")
  ...
  sum(present$girls)
   ...
   ggplot(present)+ 
    geom_line(mapping = aes(x=year,y=(boys),color="boys"))+
      geom_line(mapping = aes(x=year,y=(girls),color="girls"))+
       ylab( "Num") +
       xlab( "Year" ) +
       scale_color_discrete(name="Legend", labels=c("Boy","Girl"))+
       ggtitle(" Girl/Boys over Years")
   
       ggplot()+ 
       geom_line(present,mapping = aes(x=year,y=boys/girls),color="red")+
         geom_line(arbuthnot,mapping = aes(x=year,y=boys/girls),color="blue")
  ...
  dif
   ...
   dif<- sum(arbuthnot$boys) -sum(arbuthnot$girls)
  dif
  ...
  pr_temp<- data.frame(present$year,present$girls+present$boys)
  names(pr_temp) <- c("Year","Total_Birth")
  pr_temp[which(pr_temp$Total_Birth==max(pr_temp$Total_Birth)),]
...
Inlcuded Years are : `r unique(present$year) `
...
* From the Data points below, it can said that scale has changed very significantlym but trends are same . 

Type | Boys | Girls
------------- | -------------| ------------- 
Arbuthnot Mean| `r mean(arbuthnot$boys) ` | `r mean(arbuthnot$girls)` 
Present Mean | `r mean(present$boys)` | `r mean(present$girls) ` 
-------------------------------------- 


> We also noted that differences between boys-girls from Present dataset is `r sum(present$boys) -sum(present$girls) `, wheras for  Arbuthnot's dataset it was`r sum(arbuthnot$boys) -sum(arbuthnot$girls) `
...
#### Answer 
 ...
 + From the below plot we can say that "Arbuthnot's observation about boys being born in
    greater proportion than girls" still holds true but its in a range of 1.04 to 1.06 boy/girl.
...
+ Below plots for Birth by Year of Girls or Boys, very clearly says that we have Boys being born in more numbers compared with Girls.
...
```{r}
  ...
  most_year  <-  pr_temp[which(pr_temp$Total_Birth==max(pr_temp$Total_Birth)),1]
  most_number <-  pr_temp[which(pr_temp$Total_Birth==max(pr_temp$Total_Birth)),2]
  
```
    
+ Year `r most_year`, we see the most total number of births , i.e. `r most_number`
