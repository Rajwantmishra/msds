...
###  Answer
This is a near normal distribution with a right-skew and is unimodal. The mean is 1500, median 1442, and the range is 334 to 5642.
...
### Answer
Depending on the results of the random sample, we  find data missing the outliers reponsible for the right-skew, however these random samples are fairly reflective of the population.
```{r}
summary(samp1)
hist(samp1)
```
...
### Answer 
I would expect the mean of samp1000 (n=1000) to be closest to the area population mean.
...
### Answer
There are 5000 samples of means in sample_means50. The summary results provide the mean and median values. These values will closely reflect the original mean of the population. We are looking at a near normal distribution. The "normality" would only increase as samples size increses. We can see that mean is very close to matching with actual mean of the sample. 
```{r}
...
5.  To make sure you understand what you've done in this loop, try running a 
    ...
    element represent?
### Answer
There are 100 elements in sample_means_small. Each element represents a mean of each sample(of 50)
...
### Answer:
 When the sample size gets larger the spread decreases since most of the elements gets closer to the center, but center more or less remains unchanged.
...
mean(priceSample)
```
Point of estimate for population mean using a sample size of 50 is : `r mean(priceSample)`
...
round((((mean(priceSample)-mean(sample_means50)) / mean(priceSample)) * 100), 2)
```
