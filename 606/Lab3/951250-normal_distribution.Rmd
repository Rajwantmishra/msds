...
bd_mu_sd<- bdims%>% group_by(sex) %>% summarise(mean= mean(hgt),sd=sd(hgt), md = median(hgt))
# plyr::ddply(bdims,"sex",summarise,mean= mean(hgt))

ggplot(bdims, aes(x=hgt,fill=sex)) + 
  geom_histogram(bins = 25,binwidth=5, position="dodge" ,  color ="white")+
  geom_vline(data=bd_mu_sd, aes(xintercept=mean, color=sex),
             linetype=2)+
  ggtitle("Heights of Man and Women with Mean") + 
...
# List Median , Std Deviation, Mean
bd_mu_sd
# For Male Data Distribution 
quantile(mdims$hgt)

# For Female height distribution 
quantile(fdims$hgt)
...
### Answer
The shape of the two distributions are quite similar: both male and female heights show a symmetric and unimodal distribution. The spread of the two distributions is also similar, with most of the observations falling close to mean. They differ most notably in their centers, with a mean/medianof ~178 cm and ~165 cm for men and women respectively.
...
ggplot(bdims, aes(x=hgt , fill=sex)) + 
  ...
  ggtitle("Heights of Women(0) and  Man (1) with Median") +
  ylab("Density") + xlab("Heights") +
   geom_vline(data=bd_mu_sd, aes(xintercept=md, color=sex),
             linetype=1)+
   theme(legend.position="top")+
  facet_wrap(~sex)
...
```
### Answer 
The histogram is bell-shaped, but it a higher concentration than the normal in the middle of the distribution. This is likely due to sampling size, so saying it a normal curve would be prefered.
...
```
### Answer
The points don't fall exactly on a line, but they're quite close. The largest deviations come in the tail of the distribution. 
...
### Answer:
The qq plot for female heights is very similar to that from simulated normal data sets. In fact, several of the simulated plots show a greated deviation from linearity in the tails that does the original data. 
...
# Porb of  weigght > 50. 
# Prob Age < 20 
print("Question : Find Probability of female of Age < 20?")
muAge <-  mean(fdims$age)
sdAge <- sd(fdims$age)
xage <- 20

# Using normal distribution 
#  theoretical probability assuming the Normal Distribution
print("Using normal Distribution :")
round(pnorm(20,muAge,sdAge)*100,2)

# probability empirically from Original data
print("empirical probability ")
round((sum(fdims$age < 20) / length(fdims$age))*100,2)

print("Question :Porbability of  weight > 50 of female. \n")
...
# Using normal distribution 
#  theoretical probability assuming the Normal Distribution
round((1-pnorm(xWt ,muWt,sdWt))*100,2) # 1 - pnorm(100) as we need greater than 100 data point
print("empirical probability ")
# probability empirically from Original data
round((sum(fdims$wgt > xWt ) / length(fdims$wgt))*100,2)
